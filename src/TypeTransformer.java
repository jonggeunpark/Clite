public class TypeTransformer {
    ...
    public static Expression T (Expression e, TypeMap tm) {
        ...
        if (e instanceof Unary)
        {
            ...
            Expression t3 = T (u.term,tm);
            if (u.op.NotOp()) // ! 연산자
            {
                if(typ1 == Type.BOOL)
                return new Unary(u.op.boolMap(u.op.val),t3);
            }
            else if(u.op.NegateOp()) // - 연산자
            {
                if(typ1 == Type.FLOAT)
                    return new Unary(u.op.floatMap(u.op.val),t3);
                else if(typ1 == Type.INT)
                    return new Unary(u.op.intMap(u.op.val),t3);
            }
            else if(u.op.intOp()) // F2I , C2I 연산
            {
                if (typ1 == Type.INT)
                    return new Unary(u.op.intMap(u.op.val), t3);
                else if (typ1 == Type.FLOAT) // F2I 연산
                {
                    if (u.op.intOp() == true)
                        return new Unary(u.op.floatMap(u.op.val), t3);
                }
                else if (typ1 == Type.CHAR) // C2I 연산
                {
                    if(u.op.intOp()==true)
                        return new Unary(u.op.charMap(u.op.val),t3);
                }
            }
            else if(u.op.floatOp()) // I2F 연산
            {
                if(typ1 == Type.FLOAT)
                    return new Unary(u.op.floatMap(u.op.val),t3);
                else if (typ1 == Type.INT)
                    if(u.op.floatOp()==true)
                        return new Unary(u.op.intMap(u.op.val),t3);
            }
            else if(u.op.charOp()) // char -> int 연산
            {
                if (typ1 == Type.CHAR)
                    return new Unary(u.op.charMap(u.op.val), t3);
                else if (typ1 == Type.INT)
