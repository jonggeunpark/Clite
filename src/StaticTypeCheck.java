...
public class StaticTypeCheck {
    ...
    public static void V (Expression e, TypeMap tm) {
        ...
        if (e instanceof Unary)
        {
            ...
            V (u.term,tm);
            if(u.op.NotOp())
                check((typ1 == Type.BOOL), "type error for " + u.op);
            else if(u.op.NegateOp())
                check((typ1 == Type.INT || typ1 == Type.FLOAT), "type error for " + u.op);
            else if(u.op.intOp())
    ...
    public static void V (Statement s, TypeMap tm) {
        ...
        if(s instanceof Conditional)
        {
            ...
            if(ttype == Type.BOOL)
            {
                ...
                check( false, "non-bool type in" + c.test); // mixed를 어떤 단어로 바꿀지 모르겠음
            return;
        }
        if(s instanceof Loop)
        {
            ...
            if(ttype == Type.BOOL)
                ...
                check( false, "non-bool type in" + l.test); // mixed를 어떤 단어로 바꿀지 모르겠음
            return;
        }
